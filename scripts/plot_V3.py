...
def generate_graphs():
    ...
    for report_name, report_content in reports.items():
        tool = (
            "DockerBench" if "docker_bench" in report_name else
            "Trivy" if "trivy" in report_name else
            "Nmap" if "nmap" in report_name else
            None
        )
        ...
        # Prepare data for graph plotting based on the tool
        if tool == "DockerBench":
            status_counts = pd.DataFrame(report_content).groupby("status").size().reset_index(name="count")
            graph = px.bar(status_counts, x="status", y="count", title=f"DockerBench Report: {report_name}")

        elif tool == "Trivy":
            severity_counts = pd.DataFrame(report_content).groupby("severity").size().reset_index(name="count")
            graph = px.bar(severity_counts, x="severity", y="count", title=f"Trivy Report: {report_name}")

        elif tool == "Nmap":
            port_status_counts = pd.DataFrame(report_content).groupby("port_status").size().reset_index(name="count")
            graph = px.bar(port_status_counts, x="port_status", y="count", title=f"Nmap Report: {report_name}")

        graphs.append(dcc.Graph(figure=graph))

    return graphs if graphs else html.Div("No graph data available.")
...
app.layout = dbc.Container([
    ...
    html.Br(),
    html.Div(children=generate_graphs())
])
